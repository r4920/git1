...
async function seedUser () {
  try {
    let user = await dbService.findOne(model.user,{
      'username':'Marcellus.McDermott78',
      'isDeleted':false,
      'isActive':true
    });
    if (!user || !user.isPasswordMatch('YQvDKCsQ7ePcLQZ') ) {
      let user = {
        'password':'YQvDKCsQ7ePcLQZ',
        'username':'Marcellus.McDermott78',
    ...
    let admin = await dbService.findOne(model.user,{
      'username':'Izaiah_Haag37',
      'isDeleted':false,
      'isActive':true
    });
    if (!admin || !admin.isPasswordMatch('vtTF4vSRCsmfGEf') ) {
      let admin = {
        'password':'vtTF4vSRCsmfGEf',
        'username':'Izaiah_Haag37',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('YQvDKCsQ7ePcLQZ') && userRole){
    ...
    if (admin && admin.isPasswordMatch('vtTF4vSRCsmfGEf') && adminRole){
      let count = await dbService.count(model.userRole, {
        userId: admin.id,
        roleId: adminRole.id
      });
